...
function normalize(raw) {
  ...
  const site = { ...DEFAULT_DATA.site, ...(data.site || {}) };
  // 영역별 기획 도구 링크 — 이름과 주소가 모두 있는 것만 남긴다
  site.planners = (Array.isArray(site.planners) ? site.planners : [])
  ...
  return {
    ...
    // 명단에 없어서 못 들어온 기록 (이름·번호 오타를 찾아내려고 남긴다)
    tries: Array.isArray(data.tries) ? data.tries : [],
  };
}
...
// 수료증 공개일이 지났는지 (한국 시각 기준)
export function isCertOpen(site) {
  const d = String(site.certOpenAt || '').trim();
  if (!d) return true; // 날짜를 안 정했으면 바로 공개
  const open = new Date(`${d}T00:00:00+09:00`);
  if (Number.isNaN(open.getTime())) return true;
  return Date.now() >= open.getTime();
}

export function publicView(data) {
  // 공개일 전에는 수료증 주소를 화면으로 아예 내보내지 않는다
  const certOpen = isCertOpen(data.site);
  return {
    site: { ...data.site, certOpen, certUrl: certOpen ? data.site.certUrl : '' },
